...
if platform.system() == 'Windows':
    ...
    try:
        ...
        # If we're in win_version/cli but running on Linux, try parent directory
        parent_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'linux_version')
    ...
    def __init__(self, lang=None):
        ...
        self._strings = {
            'en': {
                ...
                'trouble2': "Check Device Manager for AX88179A adapter" if platform.system() == 'Windows' else "Check 'lsusb | grep -i asix' for the device",
                'trouble3': "Check 'ipconfig /all' for network interfaces" if platform.system() == 'Windows' else "Check 'ip link show' for network interfaces",
            ...
            'zh': {
                ...
                'trouble2': "检查设备管理器中的 AX88179A 适配器" if platform.system() == 'Windows' else "检查 'lsusb | grep -i asix' 查看设备",
                'trouble3': "检查 'ipconfig /all' 查看网络接口" if platform.system() == 'Windows' else "检查 'ip link show' 查看网络接口",
